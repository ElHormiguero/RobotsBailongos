...
{
  servo[0].pwmMINMAX(PWMmin, PWMmax);
  servo[1].pwmMINMAX(PWMmin, PWMmax);
  servo[2].pwmMINMAX(PWMmin, PWMmax);
  servo[3].pwmMINMAX(PWMmin, PWMmax);
...
void ChipE::footUp(int T, int h) {
 int pos[] = {90, 90, 90 + h, 90 - h};
 moveServos(T, pos);
}

void ChipE::bodyLeft(int T, int h) {
 int pos[] = {90, 90, 90 - h, 90 - h*2};
 int pos1[] = {90, 90, 90 - h, 90};
  moveServos(T*3/4, pos);
  moveServos(T*1/4, pos1);
}

void ChipE::bodyRight(int T, int h) {
 int pos[] = {90, 90, 90 + h*2, 90 + h};
 int pos1[] = {90, 90, 90, 90 + h};
 moveServos(T*3/4, pos);
 moveServos(T*1/4, pos1);
}

void ChipE::justFoots(int T, int Left, int Right) {
 int pos[] = {servo_position[0], servo_position[1], servo_position[2] + Left, servo_position[3] - Right};
 moveServos(T, pos);
}
