...
#define RangoJoystic 5 //Rango de salida de lectura del joystic
#define OffsetX 10
#define OffsetY 0
#define TiempoMantener 3000
...
void loop() {
  ...
  datos[4] = BotonC();
  datos[5] = BotonD();
  datos[6] = MantenerBoton(datos[5], TiempoMantener);
...
boolean MantenerBoton(boolean b, unsigned long t) {
  ...
  if (!b && estado) estado = 0; //No pulsado
  if (estado && millis() > m) return 1; //Devolvemos 1 al superar el tiempo de mantener pulsado
  else return 0; //Devolvemos 0 en caso contrario
}
